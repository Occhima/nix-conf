...
let
  body =
    ...
    let
      ...
      workspacesModule = "${uiCfg.windowManager}/workspaces";

      # Per-WM workspace settings - simplified for default variant
      ...
      # Custom power module for default variant (simpler than modules.nix version)
      defaultPowerModule = {
        format = " 󰐥 ";
        tooltip = true;
      ...
      # Simplified modules - override complex ones from modules.nix with simpler versions
      defaultModules = {
        cpu = {
          format = "{icon} {usage}%";
          tooltip = false;
...
else
  {
    config.flake.modules.homeManager.themes-guernica =
      ...
      let
        ...
        variantConfig = import ./default-variant.nix { inherit lib config uiCfg; };
      in
      {
