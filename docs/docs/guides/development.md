...
2. Contribute a named aspect — the file is discovered automatically
3. Import the aspect from a host, user, or aggregate aspect
4. Add tests if applicable

Example of a new feature aspect:

```nix
# modules/nixos/category/new-feature.nix
{ ... }:
{
  config.flake.modules.nixos.new-feature =
    { pkgs, ... }:
    {
      # Native NixOS options are normal payload; there is no
      # repository-level `enable` switch — importing the aspect
      # from a host activates it.
...
Genuine data (ports, devices, layouts) may still be declared as options;
activation must come from composition.
...
{ lib, ... }:
{
  "my helper does the thing" = {
    expr = lib.custom.isWayland { modules.system.display.type = "wayland"; };
    expected = true;
  };
}
```
