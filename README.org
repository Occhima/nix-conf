...
- [ ]  replace path references with builtins.path
  ...
  # Back to vanilla devshell
- [X] arning: Using 'builtins.derivation' to create a derivation named 'devshell-env.bash' that references the store path '/nix/store/l9nb64iii15y0nr37qrs1cfm6rlpg6gh-source' without a proper context. The resulting derivation will not have a correct store reference, so this is unreliable and may stop working in the future.
