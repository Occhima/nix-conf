...
- [X] Stylix + themes
- [X] Devenv + dev profiles  ()
- [ ] Colmena ( more bloat! )
- [X] review secrets
- [ ]  Vanilla emacs
- [ ]  Nyxt appimage
- [ ]  Podman and quadlets on home manager
- [X] Dependabot to update flake inputs
- [X] Create some nixos-containers for applications ( use rednix! )
- [X] Specializations in home-manager and nixos
- [X] Better virtualisation module
- [X] unstable pkgs as an overlay (pkgs.unstable)
- [X] evaluation warning: profile: You have set either `nixpkgs.config` or `nixpkgs.overlays` while using `home-manager.useGlobalPkgs`.
                    # This will soon not be possible. Please remove all `nixpkgs` options when using `home-manager.useGlobalPkgs`.

- [X] Use determinante.nix installer ( better warning/errors messages and more )
- [X] https://github.com/nialov/actions.nix -> githubActions alternative
